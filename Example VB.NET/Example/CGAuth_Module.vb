...
Module CGAuth_Module
    ...
    Private Function GenerateRequestID() As String
        ...
        Dim timestamp = DateTimeOffset.Now.ToUnixTimeMilliseconds().ToString()
        Dim randomBytes(15) As Byte
        Using rng As New RNGCryptoServiceProvider()
            rng.GetBytes(randomBytes)
        End Using
        Dim random = BitConverter.ToString(randomBytes).Replace("-", "")

        ' Hash for consistent length
        Using sha As SHA256 = SHA256.Create()
            Dim combined = timestamp & random
            Dim hash = sha.ComputeHash(Encoding.UTF8.GetBytes(combined))
    ...
    Public Function VerifyHMAC(data As String, hmac As String, requestId As String) As Boolean
        Using h As New HMACSHA256(Encoding.UTF8.GetBytes(API_SECRET))
            ' Combine data + request_id for HMAC calculation
            Dim combined = data & requestId
            Dim computed = BitConverter.ToString(h.ComputeHash(Encoding.UTF8.GetBytes(combined))).Replace("-", "").ToLower()
    ...
    Public Function AuthLicense(licenseKey As String, hwid As String) As JObject
        Try
            ...
            Dim requestId = GenerateRequestID()

            ' ✅ Include request_id in encrypted payload
            ...
            Using client As New WebClient()
                ...
                ' ✅ Verify timestamp (stricter: 2 minutes)
                If Math.Abs(DateTimeOffset.Now.ToUnixTimeSeconds() - timestamp) > 120 Then
                    Throw New Exception("Response expired")
                End If

                ' ✅ Verify HMAC with request_id binding
                If Not VerifyHMAC(encData, hmac, requestId) Then
                    Throw New Exception("HMAC verification failed - possible replay attack")
                End If

                ' Decrypt the response data
                Dim decrypted = DecryptPayload(encData)
                Dim result = JObject.Parse(decrypted)

                ' ✅ Verify request_id in response matches our request
                Dim responseRequestId = result("request_id")?.Value(Of String)()
                If responseRequestId IsNot Nothing AndAlso responseRequestId <> requestId Then
